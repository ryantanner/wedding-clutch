...
class VendorRouteSpec extends Specification with SessionHelper {
  ...
  "Router" should {
    ...
    "get all users if admin" in {
      running(TestServer(3333)) { 
        ...
        val adminSession = sessionToData(session(login))
    ...
    "get forbidden is getting all users not as admin" in {
      running(TestServer(3333)) {
        ...
        val normalSession = sessionToData(session(login))
